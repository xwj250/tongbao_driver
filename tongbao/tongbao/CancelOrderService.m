...
-(void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    if([[dict objectForKey:@"result"] intValue]==1){
        ...
        myAlertView = [[UIAlertView alloc]initWithTitle:@"结果" message:@"取消成功" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil];
        [myAlertView show];
        
    }else if([[dict objectForKey:@"result"] intValue] ==0){
        UIAlertView *myAlertView;
        myAlertView = [[UIAlertView alloc]initWithTitle:@"结果" message:[NSString stringWithFormat:@"取消失败，％@" ,[dict objectForKey:@"errorMsg"]] delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil];
        [myAlertView show];
