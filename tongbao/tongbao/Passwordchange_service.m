...
@implementation Passwordchange_service;
...
- (void)httpPostNoSyn:(NSString*)old second:(NSString*)now  third:(UIViewController *)control{
    ...
    NSString *param=[NSString stringWithFormat:@"token=%@&oldPassword=%@&newPassword=%@",delegate.token,old,now];
...
//        [help.navigationController pushViewController:vc animated:YES];
        ...
        myAlertView = [[UIAlertView alloc]initWithTitle:@"修改结果" message:@"失败" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil];
        [myAlertView show];
        //点击后弹出该对话框。
